...
int BUF_LEN = 0; // default to zero length
...
int main(int argc, char *argv[])
{
	...
	options.subdevice = -1;
	options.n_chan = 1;
	options.waveform = -1;	/* waveform */
	parse_options(&options, argc, argv);

	/* Use waveform to select waveform */
	fn = options.waveform;
	if(fn < 0 || fn >= NUMFUNCS){
  ...
  BUF_LEN = size / sizeof(sampl_t);
	...
	m = comedi_mark_buffer_written(dev, options.subdevice, size);
  printf("Marked %d out of %d\n", m, n);
	if(m < 0){

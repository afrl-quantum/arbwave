...
class Device(Base):
  ...
  def set_waveforms_impl(self, waveforms, clock_transitions, t_max):
    ...
    """
    # Since the timing device is only ever a timing generator, it generally does
    # not care about timing channels that are not owned by other timing devices.
    # Thus, we only pass through the clock transitions generated by this device
    # with the channel name replaced with the local naming convenction (which is
    # just the final channel number).  Thus, we use collect_prefix do help out:
    C = collect_prefix(clock_transitions, 0, 3, 3).get(str(self), {})

    used_clocks_set = set(C.keys())
    ...
    if self.isopen():
      self.guard_proxy.set_waveforms(waveforms, C, t_max)

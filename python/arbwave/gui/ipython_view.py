...
class IterableIPShell(object):
  ...
  '''
  def __init__(self,argv=[],user_module=None,user_ns=dict(), 
               cin=None, cout=None,cerr=None, input_func=None, config=Config()):
    '''
    
    
    @param argv: Command line options for IPython
    @type argv: list
    @param user_module: User global namespace.
    @type user_module: module or class (with __dict__ and __name__).
    ...
    if parse_version(IPython.release.version) >= parse_version("1.2.1"):
      self.IP = IPython.terminal.embed.InteractiveShellEmbed.instance(
              config=config, user_module=user_module, user_ns=user_ns)
    else:
...
#                                            local_ns=user_ns)
  ...
  def execute(self):
    ...
    self.continuation = self.iter_more
    prompt = self.generatePrompt(self.continuation)

    self.IP.hooks.pre_prompt_hook()
    if self.iter_more:
        try:
            prompt = self.generatePrompt(True)
        except:
...
class ConsoleView(gtk.TextView):
  ...
  def showPrompt(self):
    glib.idle_add(self._showPrompt, self.continuation)

  def _showPrompt(self, continuation):
    '''
    ...
    @type prompt: string
    '''
    prompt = self.generatePrompt(continuation)
    self._write(prompt)
  ...
  def _showReturned(self, text):
    ...
    if text:
      self._write('\n')
    self._showPrompt(self.continuation)
  ...
  def onKeyPress(self, widget, event):
    ...
    if event.keyval == gdk.KEY_Home:
      ...
      self.text_buffer.place_cursor(insert_iter)
      return True
      ...
      # emacs style
      next_iter = insert_iter.copy()
      next_iter.forward_cursor_position()
      self.text_buffer.delete(insert_iter, next_iter)
      return True
    elif event.keyval == gdk.KEY_u and event.state & gdk.ModifierType.CONTROL_MASK:
      # emacs style
      self.text_buffer.delete(start_iter, insert_iter)
      return True
...
class IPythonView(ConsoleView, IterableIPShell):
  ...
  def __init__(self, *args, **kwargs):
    ...
    self.cout = UnicodeStringIO()
    IterableIPShell.__init__(self, cout=self.cout, cerr=self.cout,
                             input_func=self.raw_input, *args, **kwargs)
  ...
  def onKeyPressExtend(self, event):
    ...
    if event.state & gdk.ModifierType.CONTROL_MASK and event.keyval == gdk.KEY_c:
      ...
      return True
         ...
         (event.keyval == gdk.KEY_p and
          event.state & gdk.ModifierType.CONTROL_MASK):
      self.changeLine(self.historyBack())
      return True
    elif event.keyval == gdk.KEY_Down or \
         (event.keyval == gdk.KEY_n and
          event.state & gdk.ModifierType.CONTROL_MASK):

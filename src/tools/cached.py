...
class threaded_property(property):
  """
  A property version for use in environments where multiple
  threads might concurrently try to access the property.
  """
  def __init__(self, func):
    super(threaded_property, self).__init__(func)
  ...
  def __get__(self, obj, cls):
    with self.lock:
      ...
      return super(threaded_property, self).__get__(obj, cls)
